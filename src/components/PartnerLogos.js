...
function PartnerLogos() {
    return (<div className="bg-slate-900/50 backdrop-blur-sm py-8">
      <div className="container mx-auto px-6">
        ...
        <div className="flex flex-wrap justify-center items-center gap-8 md:gap-12">
          {partners.map(function (partner, index) { return (<framer_motion_1.motion.a key={partner.name} href={partner.link} target="_blank" rel="noopener noreferrer" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="group relative">
              ...
              <img src={partner.logo} alt={partner.name} className="h-8 md:h-10 w-auto object-contain filter grayscale hover:grayscale-0 transition-all"/>
